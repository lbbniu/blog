...
$data=array(
	'db_logo'=>array('title'=>'logo','type'=>'upload','size'=>'60','tips'=>'大小宜为182x62'),
	'db_flash'=>array('title'=>'flash图片','type'=>'upload','size'=>'60','tips'=>'上传一张图片，大小宜为1002x308'),
	'db_index_1'=>$args=array('title'=>'介绍页面','type'=>'page','tips'=>'flash下的公司介绍，选择对应的页面'),
	'db_index_2'=>$args=array('title'=>'中部文章调用栏1','type'=>'cat'),	
	'db_index_3'=>$args=array('title'=>'中部文章调用栏2','type'=>'cat'),
	'db_index_4'=>$args=array('title'=>'底部页面调用栏1','type'=>'page','tips'=>'选择对应的页面'),	
	'db_index_5'=>$args=array('title'=>'底部页面调用栏2','type'=>'page'),
	'db_index_6'=>$args=array('title'=>'底部页面调用栏3','type'=>'page'),
	'db_index_7'=>$args=array('title'=>'底部页面调用栏4','type'=>'page'),
	'db_index_8'=>array('title'=>'底部版权文本及统计代码等','type'=>'textarea','width'=>'500px','height'=>'100px'),
	主题作者:长沙后进者网络公司,www.houjinzhe.com
);
	...
	if($_POST['submit']){
		if(!empty($data)){
			foreach($data as $key=>$result){
				...
				if($result['type']=='text') update_option($key,esc_attr($_POST[$key]));
				else update_option($key,$_POST[$key]);
			}
		}
	}
?>
...
<?php case 'editor':?>
		<tr>
			<th><?php echo $result['title']; ?></th>
			<?php $settings=array();?>
			<td><?php wp_editor('',$key,$settings);?></td>
			<?php if($result['tips']):?>	
			<td><div class="img-tips">	
				<img class="tips" id="<?php echo $key.'-img-tips' ?>" src="<?php echo bloginfo('template_url') ?>/admin-option/help.png"/>
				<div class="tips" id="<?php echo $key.'-div-tips' ?>" ><?php echo $result['tips']; ?></div>
