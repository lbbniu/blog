...
class d_postlist extends WP_Widget {
	function d_postlist() {
		...
		$this->WP_Widget( 'd_postlist', 'LbbNiu-聚合文章', $widget_ops, $control_ops );
	}
...
?>
<li><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" ><?php if( $img ){echo '<span class="thumbnail"><img src="'.imageurl(post_thumbnail_src(),'_100.64').'" alt="'.get_the_title().'" /></span>'; }else{$img = '';} ?><span class="text"><?php the_title(); ?></span><span class="muted"><?php the_time('Y-m-d');?></span><span class="muted"><?php comments_number('', '1评论', '%评论'); ?></span></a></li>
<?php
